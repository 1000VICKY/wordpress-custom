...
<!-- //メンバーシップ拡張 -->
<tr class="swpm-admin-edit-more-membership-levels">
    <th scope="row"><label for="more_membership_levels"><?php echo SwpmUtils::_('More Membership Levels'); ?></label></th>
    <td>
        <?php foreach ($levels as $key => $level): ?>
            <label><?php echo $level['alias'] ?>
                <input <?php if (in_array($level["id"], $more_membership_levels)) { ?> checked="checked" <?php } ?>type="checkbox" class="regular-text" name="more_membership_levels[]" id="more_membership_levels_<?php echo $key; ?>" value="<?php echo $level['id']; ?>">
            </label>
        <?php endforeach; ?>
    </td>
</tr>
