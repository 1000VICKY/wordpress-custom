...
class SwpmAccessControl {
    ...
    public function can_i_read_title($post){
        ...
        $more_membership_levels = json_decode($auth->get('more_membership_levels'));
        $permission = SwpmPermission::get_instance($auth->get('membership_level'));

        foreach ($more_membership_levels as $key => $value) {
            $permission = SwpmPermission::get_instance($value);
    ...
    public function filter_title($post,$title){
        print_r($post->ID);
        if (!is_a($post, 'WP_Post')) {
            //This is not a WP_Post object. So we don't want to handle it in our plugin.
            return $title;
        ...
        if(self::expired_user_has_access_to_this_page()) {
            return $title;//An expired user is viewing this page and it is a system page, so allow access.
        }
        
        if(SwpmUtils::is_first_click_free($title)) {
            return $title;//First click free is true, so allow access.
        }

        if($this->can_i_read_title($post)) {
            echo "＜---";
            return $title;//This member has access to this post, so allow access.
        } 
        echo "閲覧禁止";
        //Check and apply more tag protection.
        $more_tag_protection_value = $this->check_and_apply_more_tag_protection($post, $title);
        if(!empty($more_tag_protection_value)){
            //More tag protection was found in the post. Return the modified $title.
