...
class SwpmForm {
    ...
    protected function more_membership_levels() {
        $more_membership_levels = filter_input(INPUT_POST, 'more_membership_levels', FILTER_DEFAULT, array(
            "flags" => FILTER_REQUIRE_ARRAY
        ));
        // 入力内容が妥当な場合
        if (is_array($more_membership_levels) && count($more_membership_levels) > 0) {
            $this->sanitized['more_membership_levels'] = json_encode($more_membership_levels);
        } else {
            $this->errors['more_membership_levels'] = SwpmUtils::_('Invalid more membership levels');
            return ;
        }
    }
